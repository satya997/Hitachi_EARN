  ...
  const handleFileChange = (event) => {
    ...
    if (file) {
      ...
      if (file.type === "application/pdf") {
        // For PDFs, show the preview
        const fileURL = URL.createObjectURL(file);
        setPdfPreview(fileURL);
      } else {
        onImageUpload(URL.createObjectURL(file)); // Handle non-PDF file (if needed)
      }
    }
  };
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-opacity-40">
        <div className="relative bg-white shadow-lg w-[686px] h-[455px] border-1 border-[#D9D9D9]">
          <div className="relative flex items-center justify-center text-[#171f31] text-lg bg-[#777777] w-full h-[50px]">
            <h2 className="text-lg font-semibold">Add Document Here</h2>
            <button
              ...
              {uploadedFile ? (
                ...
                <>
                  <img
                    ...
                    className="w-6 h-6"
                  />
                  <p className="mt-2 text-black">Add document here</p>
